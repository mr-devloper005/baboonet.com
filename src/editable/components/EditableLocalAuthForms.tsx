...
const inputClass = 'h-12 rounded-xl border border-white/10 bg-white/5 px-4 text-base font-bold text-white outline-none transition placeholder:text-white/30 focus:border-[#c9a227]/50'
const buttonClass = 'inline-flex h-12 items-center justify-center rounded-full bg-[#c9a227] px-6 text-sm font-black uppercase tracking-[0.18em] text-[#0d1f14] shadow-sm transition hover:-translate-y-0.5 disabled:opacity-60'
...
export function EditableLocalLoginForm() {
  ...
  return (
    <form className="mt-6 grid gap-4" onSubmit={submit}>
      ...
      {message ? <p className={`rounded-2xl px-4 py-3 text-sm font-bold ${status === 'success' ? 'bg-emerald-900/30 text-emerald-300' : 'bg-red-900/30 text-red-300'}`}>{message}</p> : null}
